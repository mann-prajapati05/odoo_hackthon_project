...
import {
  REF_PREFIX,
  VIRTUAL_CUSTOMER_LOCATION_SHORT_CODE,
  VIRTUAL_VENDOR_LOCATION_SHORT_CODE,
...
const statusTransitions: Record<OperationStatus, OperationStatus[]> = {
...
const getVirtualLocationIds = async (tx: Prisma.TransactionClient) => {
...
const ensureWarehouse = async (tx: Prisma.TransactionClient, warehouseId: string) => {
...
export const operationService = {
  async list(query: {
    ...
    page: number;
    limit: number;
    sortBy: "createdAt" | "scheduledDate" | "refNumber";
    sortDir: "asc" | "desc";
  }) {
    const statuses = query.status?.split(",").filter(Boolean) as OperationStatus[] | undefined;
  ...
  async counts(type?: OperationType) {
  ...
  async update(id: string, input: Partial<CreateOperationInput>, userId: string) {
    return prisma.$transaction(async (tx) => {
      const existing = await tx.operation.findUnique({ where: { id } });
      if (!existing) throw new AppError("Not found", 404, "Not found");
        ...
        for (const line of input.lines) {
          await tx.operationLine.create({
            data: {
              ...
              physicalQty: line.physicalQty,
              difference:
                line.physicalQty !== undefined && line.systemQty !== undefined
  ...
  async patchStatus(id: string, nextStatus: OperationStatus, userId: string) {
    return prisma.$transaction(async (tx) => {
      const operation = await tx.operation.findUnique({ where: { id } });
      if (!operation) throw new AppError("Not found", 404, "Not found");
    ...
    return prisma.$transaction(
      async (tx) => {
        ...
        if (operation.type === OperationType.DELIVERY) {
          const details: Array<{ productId: string; productName: string; available: number; requested: number }> = [];
  ...
  async duplicate(id: string, userId: string) {
    return prisma.$transaction(async (tx) => {
      ...
      if (!source) throw new AppError("Not found", 404, "Not found");
  ...
  async lockQuantities(id: string, userId: string) {
    return prisma.$transaction(async (tx) => {
      const operation = await tx.operation.findUnique({
        where: { id },
        include: { lines: true },
      ...
      for (const line of operation.lines) {
        if (line.systemQty !== null && line.systemQty !== undefined) continue;
        ...
        await tx.operationLine.update({
          ...
          data: {
            systemQty: level?.qtyOnHand ?? 0,
            difference:
              line.physicalQty !== null && line.physicalQty !== undefined
